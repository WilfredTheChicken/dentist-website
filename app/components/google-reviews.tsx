...
export function GoogleReviews() {
  ...
  return <>
    ...
    <div className="practice-stats"><div><Users/><strong>2</strong><span>dedicated dentists</span></div><div><MapPin/><strong>1</strong><span>Staten Island office</span></div><div><Star/><strong>{data ? data.count.toLocaleString() : "5★"}</strong><span>{data ? "Google reviews" : "patient feedback"}</span></div></div>
    {data?.reviews?.length ? <div className="live-review-grid">{data.reviews.slice(0,3).map((review) => <article key={`${review.author}-${review.time}`}><div className="review-card-top"><b>{review.author}</b><span>Google</span></div><div className="star-row">{"★".repeat(Math.round(review.rating))}</div><p>“{review.text}”</p><small>{review.time}</small></article>)}</div> : <div className="live-review-grid"><article><div className="review-card-top"><b>Joyce Asabor</b><span>Google</span></div><div className="star-row">★★★★★</div><p>“The best dentist ever! Respectful, warm and chairside manner is impeccable.”</p><small>Google review</small></article><article><div className="review-card-top"><b>Natasha M.</b><span>Google</span></div><div className="star-row">★★★★★</div><p>“Dr. Daniel Kluchman proved me wrong. His knowledge, expertise and personal attention are beyond expectations.”</p><small>Google review</small></article><article className="reviews-cta"><div><b>Read more patient stories.</b><p>See recent feedback on the official Ideal Smiles Google Business Profile.</p></div><a className="button button-dark" href={destination} target="_blank" rel="noreferrer">View all Google reviews <ExternalLink size={15}/></a></article></div>}
