...
export default function Services() {
  return <main><header className="site-header section-wrap"><Link className="brand logo-brand" href="/"><img src="/ideal-smiles-logo.png" alt="Ideal Smiles Dental" /></Link><nav className="desktop-nav"><Link href="/">Home</Link><Link className="active" href="/services">Services</Link><Link href="/team">Meet Our Team</Link><Link href="/#why">Google Reviews</Link></nav><a className="button button-dark" href={zocdocUrl} target="_blank" rel="noopener noreferrer">Book a visit</a></header><section className="section-wrap services-page"><div className="services-lead"><div><p className="eyebrow">SERVICES</p><h1>Care for every stage of your smile.</h1><p className="hero-text">Find the care you need, understand the next step, and speak with our team when you are ready.</p></div><img src="/services-before-after.png" alt="Before-and-after cosmetic dentistry smile treatment" /></div><div className="service-detail-grid">{groups.map(([title, text], i) => <article key={title}><div className={`service-image service-image-${i}`} role="img" aria-label={`${title} treatment illustration`} /><h2>{title}</h2><p>{text}</p><ContactCard /></article>)}</div></section></main>;
}
